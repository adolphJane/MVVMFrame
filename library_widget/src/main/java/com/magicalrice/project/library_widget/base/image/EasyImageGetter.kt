...
package com.magicalrice.project.library_widget.base.image
...
class EasyImageGetter:Html.ImageGetter {
    ...
    fun setPlaceHolder(placeHolder:Int): EasyImageGetter {
        this.placeHolder =
            AppManager.getInstance().getApp().applicationContext.resources.getDrawable(placeHolder)
        return this
    }
    ...
    fun setError(error:Int): EasyImageGetter {
        this.error =
            AppManager.getInstance().getApp().applicationContext.resources.getDrawable(error)
        return this
    }
    ...
    override fun getDrawable(source: String?): Drawable {
        ...
        InternalAsyncTask(
            ...
            drawable,
            loader,
            WeakReference<TextView>(container)
        ).execute(source)
        return drawable
    }
    ...
    companion object {
        fun create(): EasyImageGetter =
            EasyImageGetter()

        // 判断执行环境中是否包含有glide图片解析库
        private val glideSupport =
            exist("com.bumptech.glide.Glide")
