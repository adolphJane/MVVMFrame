...
object LocationUtils {
    ...
    private val MIN_TIME_BETWEEN_UPDATES: Long = 0
    ...
    fun getLocation(context: Context, listener: LocationListener): Location? {
        ...
        try {
            ...
            if (!isLocationEnabled()) {
                ...
                ToastUtils.showShort(" you have to open GPS or INTERNET")
            } else {
                if (isLocationEnabled()) {
    ...
    fun isGpsEnabled(): Boolean {
        val lm =
            AppManager.getInstance().getApp().getSystemService(Context.LOCATION_SERVICE) as LocationManager
    ...
    fun register(minTime: Long, minDistance: Long, listener: OnLocationChangeListener?): Boolean {
        ...
        if (mLocationManager?.isProviderEnabled(LocationManager.NETWORK_PROVIDER) == false && mLocationManager?.isProviderEnabled(
                LocationManager.GPS_PROVIDER
            ) == false
        ) {
    ...
    fun getAddress(latitude: Double, longitude: Double): Address? {
        val geocoder = Geocoder(AppManager.getInstance().getApp(), Locale.getDefault())
        try {
