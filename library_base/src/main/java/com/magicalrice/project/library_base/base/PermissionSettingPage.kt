...
/**
 * @package com.ixiye.project.baseutils
 * @author Adolph
 * @date 2019-05-16 Thu
 * @description 跳转应用权限设置页
 */
...
object PermissionSettingPage {
    ...
    fun start(context: Context, newTask: Boolean) {
        ...
        when {
            MARK.contains("huawei") -> intent = huawei(context)
            MARK.contains("xiaomi") -> intent = xiaomi(context)
            MARK.contains("oppo") -> intent = oppo(context)
            MARK.contains("vivo") -> intent = vivo(context)
            MARK.contains("meizu") -> intent = meizu(context)
        }
