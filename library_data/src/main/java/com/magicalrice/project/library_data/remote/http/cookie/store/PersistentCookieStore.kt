...
class PersistentCookieStore :
    CookieStore {
    private var cookies: HashMap<String, ConcurrentHashMap<String, Cookie>> = hashMapOf()
    ...
    override fun saveCookie(url: HttpUrl, cookie: List<Cookie>) {
        ...
        for (bean in cookie) {
            ...
            if (isCookieExpired(
                ...
                removeCookie(url, bean)
            } else {
                saveCookie(url, bean, getCookieToken(bean))
            }
        }
    }
    ...
    override fun removeCookie(url: HttpUrl, cookie: Cookie): Boolean {
        ...
        if (cookies.containsKey(url.host()) && cookies[url.host()]?.containsKey(name) == true) {
            //内存移除
